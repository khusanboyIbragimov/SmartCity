    ...
    render() {
        ...
        return(
            <div className='container'>
                <div className='row'>
                    <div className='col-sm-12 panel-panel'>
                        <div className='row'>
                        <h4 className="h4">{title}</h4>
                            ...
                            <div className="col-sm-1"><br /><br />
                                <span style={{ color: "grey", fontSize: "10px", textAlign: "center" }}>
                                    {Moment(news_timestamp).format("LLLL")}<br />{fullname}
                                </span>
                            </div>
                                        ...
                                        <div className='col-sm-6'>
                                            <form onSubmit={handleSubmitRightNews} id={news_id}>
                                                {userId && users_who_agree.indexOf(userId.toString()) !== -1 ?
                                                    <button className='btn btn-success' disabled>ҳақиқат{" "}{rightees}</button> :
                                                    <button className='btn btn-success'>ҳақиқат{" "}{rightees}</button>}
                                            </form>
                                        </div><br /><br />
                                        <div className='col-sm-6'>
                                            <form onSubmit={handleSubmitWrongNews} id={news_id}>
                                                {userId && users_who_disagree.indexOf(userId.toString()) !== -1 ?
                                                    <button className='btn btn-danger' disabled>ғийбат&nbsp;&nbsp;{" "}{wrongees}</button> :
                                                    <button className='btn btn-danger'>ғийбат&nbsp;&nbsp;{" "}{wrongees}</button>}
                                            </form>
                                        </div>
                                    </div> :
                                        <div className="row">
                            ...
                            <div className='col-sm-5'>
                                <div key={news_id}>
                                    ...
                                    <div style={{ textAlign: 'left' }}>{text.length > 150 ? <div className="text-justify">{text}</div>  : <div>{text}<br /><br />
                                        <blockquote style={{ fontSize: "9px", color: "grey" }} className={`text-justify ${showHistoryText? "show": "hide"} pagination`}  onClick={this.handleClickHistoryText}>
